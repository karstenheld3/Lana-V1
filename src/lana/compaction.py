...
# Pre-call fit check (EC-30): would projected + estimated tool batch exceed the generator's max_input?
def fits_window(messages: list[Message], max_input: int, batch_estimate: int) -> bool:
  return project_from_messages(messages) + batch_estimate <= max_input
...
def split_sections(text: str) -> tuple[str, str, str]:
  ...
  if not any(sections.values()):
    sections["objective"] = text  # EC-32: fallback to objective (most critical section), not session summary
    dlog("app", "compaction_fallback", reason="no section headers found")
...
def render_transcript(messages: list[Message], limit_chars: int = 100000) -> str:
  ...
  # EC-34: preserve first user message (objective) before tail truncation
  first_user = next((f"[USER] {m.content[:2000]}" for m in messages if m.role == "user"), "")
  if not first_user: return text[-limit_chars:]
  budget = limit_chars - len(first_user) - 10  # 10 chars for separator
  if budget <= 0: return first_user[:limit_chars]
  return first_user + "\n...\n" + text[-budget:]
...
def make_compactor(app: AppConfig):
  async def compact(agent) -> AsyncIterator:
    ...
    try:  # FR-16 CR-04: the whole compact body shares the EC-17 warn-and-continue semantics
      ...
      # Tail integrity: strip assistant messages whose tool_calls lack matching tool results in the tail (EC-31)
      while tail and tail[0].role == "assistant" and tail[0].tool_calls:
        call_ids = {call.id for call in tail[0].tool_calls}
        result_ids = {msg.tool_call_id for msg in tail if msg.role == "tool"}
        if call_ids <= result_ids: break  # all tool calls have matching results
        tail = tail[1:]
        while tail and tail[0].role == "tool": tail = tail[1:]  # strip newly orphaned tool results
