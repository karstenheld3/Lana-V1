...
EXECUTORS = {**_EXECUTORS_BASE, **_EXECUTORS_LEGACY_SEARCH}  # backward compat for test conftest import
...
def build_arg_parser() -> argparse.ArgumentParser:
  ...
  parser.add_argument("--config", metavar="PATH", help="config file override (env AGENT_CONFIG)")
  parser.add_argument("--policy", choices=["manual", "auto", "turbo"], help="execution policy override")
  parser.add_argument("--debug", action="store_true", help="write redacted request/response JSON to .agent-data/logs/")
  ...
  parser.add_argument("--app-dir", metavar="PATH", help="base directory for config, prompt library, and data (DD-25; env AGENT_APP_DIR; default: exe parent or cwd)")
...
def resolve_app_dir(args) -> Path | None:
  """DD-25 EC-30: --app-dir > env AGENT_APP_DIR > PYAPP exe parent (if packaged) > None (config.py defaults to workspace)."""
  raw = getattr(args, "app_dir", None) or os.environ.get("AGENT_APP_DIR") or None
...
def build_runtime(args, workspace: Path, interactive: bool, app_dir: Path | None = None):
  ...
  config_override = args.config or os.environ.get("AGENT_CONFIG") or None
  ...
  tools_dir = (app_dir or workspace) / ".agent-tools"
...
# _loop: persistent event loop from repl(); None = headless, uses asyncio.run() per call.
def run_one_prompt(agent: Agent, renderer: Renderer | None, text: str, jsonl_output: bool, *, _loop: asyncio.AbstractEventLoop | None = None) -> str | None:
  async def consume():
  ...
  try:
    if _loop is None:
      asyncio.run(consume())  # headless: single-shot, cleanup at exit is fine
    else:
      task = _loop.create_task(consume())  # REPL: reuse the session loop - no shutdown_asyncgens() on each cancel
      _loop.run_until_complete(task)
  except KeyboardInterrupt:
    if _loop is not None:
      task.cancel()  # type: ignore[possibly-undefined]
      async def _drain(): await asyncio.wait({task}, timeout=0.1)  # type: ignore[possibly-undefined]
      with contextlib.suppress(Exception): _loop.run_until_complete(_drain())
    if renderer is not None: renderer.end_stream(); renderer.stop_status()  # BG-0001: stop the "generator thinking..." spinner
...
def repl(agent: Agent, cost_tracker: CostTracker, prompt_system) -> int:
  ...
  loop = asyncio.new_event_loop()  # one loop for the entire REPL session; CTRL+C cancels per-turn tasks without shutdown_asyncgens() block
  ...
  try:
    while True:
      ...
      try:
        run_one_prompt(agent, renderer, text, jsonl_output=False, _loop=loop)
      except UnknownWorkflowError as error:
        print(str(error))
    ...
    if pending:
      with contextlib.suppress(Exception): loop.run_until_complete(asyncio.gather(*pending, return_exceptions=True))
    with contextlib.suppress(Exception): loop.run_until_complete(loop.shutdown_asyncgens())
    with contextlib.suppress(Exception): loop.run_until_complete(loop.shutdown_default_executor())
    loop.close()
...
def main() -> int:
  ...
  try:
    ...
    return EXIT_CONFIG
  except Exception as error:  # FR-16 CR-04: any other startup failure is self-contained, never a raw traceback
    print(f"ERROR: startup failed: {type(error).__name__}: {error}.\n  HINT: check config files and prompt system for syntax errors. Run with --debug for diagnostic logs.", file=sys.stderr)
    return EXIT_CONFIG
