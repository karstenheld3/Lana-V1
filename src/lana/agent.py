...
from lana.compaction import fits_window
...
# Expand /name input into slash-command workflow wrapping (FR-05, OQ-22); returns (content, workflow_name)
...
class Agent:
  def __init__(self, app: AppConfig, prompt_system: PromptSystem, system_prompt: str, registry: ToolRegistry, tool_context: ToolContext, session: SessionStore,
    ...
    self.final_text = ""
    self._skip_pre_call_check = False            # EC-30: skip pre-call check when post-turn compaction just fired
  ...
  def approval_needed(self, call: ToolCall, args: dict) -> tuple[bool, str, str]:
    ...
    if call.name in WRITE_TOOLS:
      ...
      return False, "", ""
    if call.name == "read_url_content": return True, "read_url_content", args.get("Url", "")
    return False, "", ""
  ...
  async def run_prompt(self, user_input: str) -> AsyncIterator:
    ...
    while True:
      # Pre-call context fit check (EC-30): compact before API call if projected + batch would exceed max_input
      # Skip on first iteration - no tool results yet, compaction can't help
      if self.compactor is not None and not first_iteration:
        if self._skip_pre_call_check:
          self._skip_pre_call_check = False
        else:
          batch_estimate = self.app.lana.max_tool_calls_per_prompt * self.app.lana.tool_result_max_chars // 4
          if not fits_window(self.messages, role.max_input, batch_estimate):
      ...
      finished = TurnFinished(role="generator", input_tokens=usage.input_tokens if usage else 0, output_tokens=usage.output_tokens if usage else 0,
                              cache_read_tokens=usage.cache_read_tokens if usage else 0, cache_write_tokens=usage.cache_write_tokens if usage else 0,
                              cost_usd=cost, thinking_payloads=payloads)
      if not tool_calls:
        yield self.emit(finished)
        async for event in self.maybe_compact():
          if event.type == "checkpoint_created": self._skip_pre_call_check = True
          yield event
        break
      ...
      async for event in self.maybe_compact():
        if event.type == "checkpoint_created": self._skip_pre_call_check = True
        yield event  # FR-07: checked after EACH turn, not only post-prompt
