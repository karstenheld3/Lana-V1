...
# EC-35: extract HTTP status code and response body excerpt from SDK exceptions
def format_provider_error(provider: str, error: Exception) -> str:
  status = getattr(error, "status_code", None)
  body = None
  response = getattr(error, "response", None)
  if response is not None: body = getattr(response, "text", None) or str(response)[:200]
  if body is None: body = getattr(error, "body", None)
  if body is not None and isinstance(body, bytes): body = body.decode("utf-8", errors="replace")
  parts = [f"{provider} API error"]
  if status is not None: parts.append(str(status))
  if body: parts.append(str(body)[:200])
  elif str(error): parts.append(str(error)[:200])
  return ": ".join(parts)
