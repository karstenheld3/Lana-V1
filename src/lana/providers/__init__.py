...
def get_adapter(role: ResolvedRole, app: AppConfig):
  ...
  elif role.provider == "zai":
    from lana.providers.zai_adapter import ZAIAdapter  # deferred: SDK client construction only when a live role needs it
    _ADAPTER_CACHE["zai"] = ZAIAdapter(api_key=app.keys["zai"], debug_dir=getattr(app, "debug_dir", None))
  else:
    raise ProviderError(f"Unknown provider '{role.provider}' for model '{role.model_id}' - expected openai, anthropic, or zai")
