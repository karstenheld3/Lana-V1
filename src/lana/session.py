...
class _Projector:
  ...
  def apply(self, event) -> None:
    ...
    if kind == "user_message":
      ...
      usage = Usage(input_tokens=event.input_tokens, output_tokens=event.output_tokens, cache_read_tokens=event.cache_read_tokens, cache_write_tokens=event.cache_write_tokens)
...
def resume(path: Path) -> ResumedState:
  ...
  for line in Path(path).read_text(encoding="utf-8").splitlines():
    ...
    if event.type == "turn_finished":
      ...
      state.usage_by_role[event.role] = usage.add(Usage(input_tokens=event.input_tokens, output_tokens=event.output_tokens, cache_read_tokens=event.cache_read_tokens, cache_write_tokens=event.cache_write_tokens))
