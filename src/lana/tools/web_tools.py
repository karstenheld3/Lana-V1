...
CHUNK_CHARS = 5000  # [ASSUMED - matches observed 2-8 KB chunk cost range]
...
def chunks_dir(context: ToolContext) -> Path:
  return (context.data_dir or Path(context.workspace) / ".agent-data") / "chunks"
...
# Render the documented 5-result format (FR-13)
...
def execute_search_web(args: dict, context: ToolContext) -> str:
  ...
  if role is None: raise ToolError("search_web unavailable: no 'websearch' role configured in agent-config.json.")
