...
class AcpServer:
  ...
  async def handle_session_load(self, request: Request) -> None:
    ...
    config_override = self.args.config or os.environ.get("AGENT_CONFIG") or None
    from lana.config import load_lana_config
    from lana.cli import resolve_app_dir
    app_dir = resolve_app_dir(self.args)
    temp_app = load_lana_config(Path(cwd), Path(config_override) if config_override else None, require_keys=False, app_dir=app_dir)
  ...
  async def build_session_runtime(self, method: str, args, cwd: str):
    from lana.cli import build_runtime, resolve_app_dir  # lazy: lana.cli is the composition root - importing it at module load would couple the frontends
    app_dir = resolve_app_dir(args)  # DD-25: app dir from CLI flag, not from ACP cwd
    def build():
      with contextlib.redirect_stdout(sys.stderr):  # redirect inside the callable - it must wrap the executor thread's prints
        return build_runtime(args, Path(cwd), interactive=False, app_dir=app_dir)
    try:
