...
ENV_KEY_NAMES = {"openai": "OPENAI_API_KEY", "anthropic": "ANTHROPIC_API_KEY", "zai": "ZAI_API_KEY"}
PROVIDER_DISPLAY = {"openai": "OpenAI", "anthropic": "Anthropic", "zai": "Z.AI"}

# Zero-setup default roles (FR-16, DD-02/DD-23): written to a missing DEFAULT config path at startup
DEFAULT_ROLES = {
  "generator": {"model_id": "glm-5.2", "effort": "high"},
...
class AppConfig:
  ...
  agent_folder: Path = Path(".agent")  # resolved absolute agent folder path (prompt system)
  data_dir: Path = Path(".agent-data")  # resolved absolute runtime data directory (sessions, logs, chunks)
...
def read_json(path: Path) -> dict:
  try:
    ...
    raise ConfigError(f"Config file not found: '{path}'.\n  HINT: create it or pass --config <path> (env AGENT_CONFIG).") from None
    ...
    raise ConfigError(f"File '{path}' is not valid UTF-8: {error}.\n  HINT: re-save the file as UTF-8 encoding.") from None
...
def materialize_bundled_agent(target: Path) -> bool:
  ...
  try:
    ...
    raise ConfigError(f"Cannot copy bundled prompt library to '{target}': {error}.\n  HINT: check folder permissions or remove the existing folder and restart.") from None
  return True


# Materialize bundled tools (rg.bin -> rg.exe) into app_dir/.agent-tools/; existing files stay untouched
...
def parse_key_file(path: Path) -> dict[str, str]:
  ...
  try:
    ...
    raise ConfigError(f"Key file '{path}' is not valid UTF-8.\n  HINT: re-save the file as UTF-8 encoding.") from None
...
def translate_effort(method: str, effort: str, prefix_entry: dict, mapping: dict) -> dict:
  ...
  if factors is None:
    ...
    raise ConfigError(f"Unknown effort level '{effort}' in 'agent-config.json'.\n  HINT: use one of: {valid}.")
...
def resolve_role(role_name: str, spec: RoleSpec, registry: dict, mapping: dict) -> ResolvedRole:
  ...
  try:
    ...
    raise ConfigError(f"Role '{role_name}' model '{spec.model_id}' has a malformed entry in 'config/model-registry.json': missing key '{error}'.\n  HINT: check the prefix entry for this model family.") from None
...
def load_lana_config(workspace: Path, config_path: Optional[Path] = None, require_keys: bool = True, app_dir: Optional[Path] = None) -> AppConfig:
  ...
  └── config_path default: <app_dir>/.agent-data/config/agent-config.json (override via --config / AGENT_CONFIG)
  ...
  if config_path is None:
    config_path = app_dir / ".agent-data" / "config" / "agent-config.json"
