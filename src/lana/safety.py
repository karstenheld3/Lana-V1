...
def is_wrapper_invocation(command_line: str) -> bool:
  ...
  # EC-33: match flag as a whitespace-delimited word, flag at end of line, or flag immediately before a quote
  words = lowered.split()
  for flag in WRAPPER_EXECUTION_FLAGS:
    if flag in words: return True  # flag as a standalone word (handles any casing)
    if lowered.rstrip().endswith(f" {flag}"): return True  # flag at end of line with trailing spaces
    if f" {flag}\"" in lowered or f" {flag}'" in lowered: return True  # flag immediately before a quote
  return False
