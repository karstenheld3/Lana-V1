# Lana build pipeline (LANADIST-SP01, LANADIST-IP01): sync .agent/ -> bundled, wheel -> PyApp single binary -> sign -> checksum -> cleanup.
...
if (-not (Test-Path $VenvPy)) {
  Write-Host "  .venv not found - running InstallBuildTools.bat..."
  $installBat = Join-Path $RootDir 'InstallBuildTools.bat'
  if (-not (Test-Path $installBat)) { Fail "InstallBuildTools.bat not found (EC-02)." }
...
if (-not (Test-Path $lanaLibrary -PathType Container)) { Fail ".agent prompt library missing - bundle would lose the agent library (EC-13)." }
...
robocopy (Join-Path $RootDir '.agent-data\config') $bundleConfig 'model-registry.json' 'model-parameter-mapping.json' 'model-pricing.json' /NJH /NJS /NDL /NC /NS /NP | Out-Null
...
$rgSource = Join-Path $RootDir '.agent-tools\rg.exe'
$toolsLabel = 'no tools'
