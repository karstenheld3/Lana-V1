...
# Model data files only - no agent-config.json (zero-setup creates it)
def write_model_data_only(base_path):
  config_dir = base_path / ".agent-data" / "config"
...
def test_tc01_zero_setup_creates_and_reports(tmp_path, monkeypatch, capsys, populated_bundle):
  ...
  assert (tmp_path / ".agent-data" / "config" / "agent-config.json").is_file()
  assert (tmp_path / ".agent-data" / "config" / ".api-keys.txt").is_file()  # keyless template (LANADIST-DD-09)
  assert (tmp_path / ".agent-data" / "sessions").is_dir()
  assert (tmp_path / ".agent" / "rules").is_dir() and (tmp_path / ".agent" / "workflows").is_dir() and (tmp_path / ".agent" / "skills").is_dir()
  ...
  created_config = json.loads((tmp_path / ".agent-data" / "config" / "agent-config.json").read_text(encoding="utf-8"))
...
def test_tc02_explicit_config_missing_stays_error(tmp_path):
  missing = tmp_path / "elsewhere" / "agent-config.json"
...
def test_tc03_data_dir_blocked_by_file(tmp_path, monkeypatch, capsys):
  from lana.cli import build_runtime
  from lana.providers import reset_adapter_cache
  # Write config to a separate dir so .agent-data can be blocked by a file
  config_dir = tmp_path / "alt_config"
  config_dir.mkdir(parents=True)
  from tests.conftest import TEST_MAPPING, TEST_PRICING, TEST_REGISTRY, DEFAULT_LANA_CONFIG
  (config_dir / "agent-config.json").write_text(json.dumps(DEFAULT_LANA_CONFIG, indent=2), encoding="utf-8")
  ...
  (tmp_path / ".agent-data").write_text("i am a file", encoding="utf-8")
  monkeypatch.setenv("LANA_SCRIPTED_ADAPTER", str(tmp_path / "unused-script.jsonl"))
  monkeypatch.setenv("AGENT_CONFIG", str(config_dir / "agent-config.json"))
  reset_adapter_cache()
  with pytest.raises(OSError):
    build_runtime(make_args("--config", str(config_dir / "agent-config.json")), tmp_path, interactive=False)
  reset_adapter_cache()
