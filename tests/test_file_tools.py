...
def test_search_tools_skip_ignored_directories(tmp_path, context):
  ...
  for noise_dir in (".git", "node_modules", "__pycache__", ".agent", ".agent-data"):
...
# ripgrep integration: run the same search tests with rg when .agent-tools/rg.exe is available
from lana.tools.file_tools import _find_rg, execute_search

@pytest.fixture
def rg_context(tmp_path):
  """Context with tools_dir pointing to workspace .agent-tools (skipped if rg not present)."""
  workspace_tools = Path(__file__).resolve().parent.parent / ".agent-tools"
  ctx = ToolContext(workspace=tmp_path, tool_result_max_chars=50000, tools_dir=workspace_tools)
  if _find_rg(ctx) is None: pytest.skip("rg not available in .agent-tools/")
  return ctx
