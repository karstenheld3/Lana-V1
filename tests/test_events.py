...
ALL_EVENTS = [
  ...
  events.TurnFinished(input_tokens=21050, output_tokens=412, cache_read_tokens=18200, cache_write_tokens=500, cost_usd=0.0164),
...
def test_timestamp_format():
  ...
  assert len(event.ts) == 23 and event.ts[4] == "-" and event.ts[10] == " " and event.ts[13] == ":" and event.ts[19] == "."


# FR-08: cache_write_tokens field on TurnFinished (default 0 for backward compat)
def test_turn_finished_cache_write_tokens_round_trip():
  event = events.TurnFinished(input_tokens=100, output_tokens=20, cache_read_tokens=50, cache_write_tokens=500)
  restored = from_jsonl(event.to_jsonl())
  assert restored.cache_write_tokens == 500


def test_turn_finished_cache_write_tokens_default_zero():
  event = events.TurnFinished(input_tokens=100, output_tokens=20)
  assert event.cache_write_tokens == 0  # default on the model
  restored = from_jsonl(event.to_jsonl())
  assert restored.cache_write_tokens == 0  # default fills in on deserialization
