...
# TC-04c: Z.AI provider resolves with reasoning_effort method and high effort
def test_tc04c_zai_provider_resolves(tmp_path, clean_key_env):
  write_config_dir(tmp_path, lana_overrides={"roles": {
  ...
  app = load_lana_config(tmp_path)
  gen = app.roles["generator"]
  assert gen.provider == "zai" and gen.method == "reasoning_effort"
  assert gen.params == {"reasoning_effort": "high"}
  assert gen.max_input == 1000000 and gen.max_output == 16384
  assert app.keys == {"zai": "sk-test-zai", "openai": "sk-test-openai"}
...
# TC-06: malformed agent-config.json -> error with line context
def test_tc06_malformed_json_line_context(tmp_path, clean_key_env):
  config_dir = write_config_dir(tmp_path)
  (config_dir / "agent-config.json").write_text('{\n  "roles": {\n  BROKEN\n}\n', encoding="utf-8")
  with pytest.raises(ConfigError) as error: load_lana_config(tmp_path)
  assert "line 3" in str(error.value) and "agent-config.json" in str(error.value)
...
def test_tc68_install_root_separation(tmp_path, clean_key_env):
  ...
  assert app.agent_folder == (install_dir / ".agent").resolve()
  assert app.data_dir == (install_dir / ".agent-data").resolve()
  # workspace stays as passed (for tool operations)
  assert app.workspace == workspace_dir
  # config_dir is inside app_dir
  assert app.config_dir == install_dir / ".agent-data" / "config"
