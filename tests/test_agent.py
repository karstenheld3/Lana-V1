...
# TC-52: is_context_overflow detection (EC-20)
def test_tc52_is_context_overflow_detection():
  from lana.agent import is_context_overflow
  ...
  assert not is_context_overflow("internal server error")
  assert not is_context_overflow("rate limit exceeded")


# TC-53: agent with no cost_fn (None handling, no crash)
def test_tc53_agent_no_cost_fn(agent_factory):
  from lana.agent import Agent
  turns = [{"text": "hello", "usage": {"input": 100, "output": 10}}]
  agent = agent_factory(turns)
  agent.cost_fn = None  # simulate no cost tracking
  events = collect_events(agent, "go")
  assert agent.final_text == "hello"
