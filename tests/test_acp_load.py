...
def test_tc36_recorded_environment_authority(loaded_workspace):
  ...
  session_file = workspace / ".agent-data" / "sessions" / f"{session_id}.jsonl"
  proc = LanaProc(workspace, script_path=script)
...
def test_tc37_unknown_and_legacy(loaded_workspace):
  ...
  legacy_file = workspace / ".agent-data" / "sessions" / "legacy_0000.jsonl"
  ...
  try:
    ...
    assert "error" in response and ".agent-data" in response["error"]["message"]  # EC-17: self-contained
