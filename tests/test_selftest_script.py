...
SCRIPT_PATH = Path(__file__).resolve().parents[1] / ".agent" / "skills" / "selftest" / "selftest.py"
...
def make_ctx(selftest, tmp_path, registry=None, pricing=None, keys=None, **arg_overrides):
  ...
  ctx = selftest.Context(workspace=tmp_path, config_dir=tmp_path / ".agent-data" / "config", args=args)
...
def make_workspace(tmp_path, with_prompt_system=True):
  ...
  if with_prompt_system:
    write_prompt_system(tmp_path / ".agent",
      rules={"r.md": "rule"},
      workflows={"prime": "---\ndescription: Prime\n---\n# Prime"},
      skills={"demo": ("---\nname: demo\n---\n# Demo", {})})
  return tmp_path
...
def latest_results(workspace):
  runs = sorted((workspace / ".agent-data" / "selftest").iterdir())
...
def test_tc05_discovery_filters_enabled_available(selftest, tmp_path):
  ...
  assert ids == ["claude-sonnet-4-5-20250929", "gpt-4.1-mini", "gpt-5.5", "glm-5.2"]  # -pro disabled, untested excluded
...
def test_tc09_missing_pricing_fails_check_not_run(selftest, tmp_path):
  workspace = make_workspace(tmp_path)
  (workspace / ".agent-data" / "config" / "model-pricing.json").unlink()
...
def test_tc10_skill_without_skill_md_fails_named(selftest, tmp_path):
  workspace = make_workspace(tmp_path)
  (workspace / ".agent" / "skills" / "broken-skill").mkdir(parents=True)
...
def test_tc13_exit_codes(selftest, monkeypatch, tmp_path):
  ...
  broken = make_workspace(tmp_path / "broken", with_prompt_system=False)  # no .agent -> category 03 fails
...
def test_tc19_effort_matrix_selection(selftest, tmp_path):
  ...
  assert representatives["reasoning_effort"]["model_id"] == "glm-5.2"  # cheapest reasoning_effort model (1.40/1M vs gpt-5.5 no pricing)
  assert representatives["reasoning_effort"]["prefix_entry"]["effort"] == ["none", "minimal", "low", "medium", "high", "xhigh", "max"]  # from prefix array
