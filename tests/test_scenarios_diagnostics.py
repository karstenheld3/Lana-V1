...
IPPS_PATH = Path(__file__).resolve().parent.parent / ".agent"
...
def test_tp01_tc08_ipps_startup(tmp_path):
  ...
  proc = LanaProc(workspace, config_path=config_dir / "agent-config.json", script_path=write_script(workspace / "s.jsonl", []))
...
def test_tp01_tc09_debug_and_no_secret_leak(tmp_path, monkeypatch):
  ...
  logs_dir = proc.workspace / ".agent-data" / "logs"
  ...
  for event in proc.events(env_result):
    assert re.fullmatch(r"\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}(\.\d{3})?", event.ts)  # NFR-04 timestamps (optional ms)
