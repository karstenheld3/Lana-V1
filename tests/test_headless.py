...
def cli_workspace(tmp_path):
  ...
  return workspace, config_dir / "agent-config.json"
...
def test_tc51_missing_config_exit_2(tmp_path):
  ...
  workspace.mkdir()
  proc = LanaProc(workspace, config_path=workspace / ".agent-data" / "config" / "agent-config.json", script_path=None)
  script_free_env_result = proc.run_headless("hello", output_format="text")
  assert script_free_env_result.returncode == 2
  assert "agent-config.json" in script_free_env_result.stderr and "HINT:" in script_free_env_result.stderr
...
def test_exit_code_4_on_limit_stop(cli_workspace):
  ...
  limited = workspace / ".agent-data" / "config" / "agent-config-limited.json"
