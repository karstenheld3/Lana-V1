...
def write_session(workspace, name: str, event_list) -> str:
  store = SessionStore(workspace / ".agent-data" / "sessions" / f"{name}.jsonl")
...
def test_tc61_scoring_and_cap(tmp_path, context, demo_session):
  result = execute_trajectory_search({"ID": demo_session, "Query": "crawler refactor", "SearchType": "conversation"}, context)
  ...
  capped = execute_trajectory_search({"ID": big_id, "Query": "needle", "SearchType": "conversation"}, context)
...
def test_tc62_empty_query_and_id_resolution(tmp_path, context, demo_session):
  by_stem = execute_trajectory_search({"ID": demo_session, "Query": "", "SearchType": "conversation"}, context)
  ...
  by_exact = execute_trajectory_search({"ID": demo_session + ".jsonl", "Query": "", "SearchType": "conversation"}, context)
  by_prefix = execute_trajectory_search({"ID": "2026-08-30_12", "Query": "", "SearchType": "conversation"}, context)
...
def test_tc63_error_paths(tmp_path, context, demo_session):
  with pytest.raises(ToolError) as error: execute_trajectory_search({"ID": "nope", "Query": "x", "SearchType": "conversation"}, context)
  ...
  with pytest.raises(ToolError) as error: execute_trajectory_search({"ID": "2026-08-30", "Query": "x", "SearchType": "conversation"}, context)
  ...
  with pytest.raises(ToolError) as error: execute_trajectory_search({"ID": "any", "Query": "x", "SearchType": "conversation"}, empty_workspace_context)
  assert "No sessions folder" in str(error.value)


def test_no_match_message(tmp_path, context, demo_session):
  result = execute_trajectory_search({"ID": demo_session, "Query": "zzz_nothing_matches", "SearchType": "conversation"}, context)
