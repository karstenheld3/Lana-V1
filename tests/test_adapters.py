...
# TC-46: format_provider_error extracts status code and body excerpt (EC-35, FR-06)
def test_tc46_format_provider_error_with_status():
  from lana.providers.base import format_provider_error
  class FakeError(Exception):
    status_code = 429
    body = '{"error": "Rate limit exceeded"}'
  msg = format_provider_error("OpenAI", FakeError("rate limited"))
  assert "429" in msg
  assert "Rate limit exceeded" in msg
  assert msg.startswith("OpenAI API error: 429")


# TC-46b: format_provider_error with response.text
def test_tc46b_format_provider_error_with_response():
  from lana.providers.base import format_provider_error
  class FakeResponse:
  ...
  msg = format_provider_error("Anthropic", FakeError("server error"))
  assert "500" in msg
  assert "Internal server error" in msg


# TC-46c: format_provider_error falls back to str(error) when no status/body
def test_tc46c_format_provider_error_fallback():
  from lana.providers.base import format_provider_error
  msg = format_provider_error("OpenAI", ValueError("something went wrong"))
  assert "OpenAI API error" in msg
  assert "something went wrong" in msg
  assert "429" not in msg  # no status code present
