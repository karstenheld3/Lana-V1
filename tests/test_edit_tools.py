...
# TC-53: edit after Lana's own write_to_file passes the gate (RF-10 self-edit chain)
def test_tc53_self_edit_chain_after_write(tmp_path, context):
  target = tmp_path / "new_file.txt"
  execute_write_to_file({"TargetFile": str(target), "CodeContent": "line1\nline2\n", "EmptyFile": False}, context)
  execute_edit({"file_path": str(target), "old_string": "line1", "new_string": "LINE1"}, context)
  execute_edit({"file_path": str(target), "old_string": "line2", "new_string": "LINE2"}, context)
