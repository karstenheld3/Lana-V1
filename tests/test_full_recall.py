...
def test_tc64_session_started_first_line_with_environment(tmp_path):
  ...
  assert len(tool_names) == 15 and "read_file" in tool_names and "trajectory_search" in tool_names  # unified_file_search_tool=True replaces grep_search+find_by_name with search
...
def test_tc67_legacy_session_fallback(tmp_path):
  ...
  sessions_dir = proc.workspace / ".agent-data" / "sessions"
