...
# TC-54: zero usage returns 0.0 (not None)
def test_tc54_zero_usage_returns_zero(workspace):
  tracker = make_tracker(workspace)
  zero = Usage(input_tokens=0, output_tokens=0)
  assert tracker.turn_cost("generator", zero) == 0.0
  assert tracker.record("generator", zero) == 0.0
  assert tracker.by_role["generator"].cost_usd == 0.0
  assert tracker.by_role["generator"].priced is True
