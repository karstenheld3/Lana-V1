...
**Goal**: Implement the `/selftest` workflow, skill, and runner script specified in `_SPEC_SELFTEST.md [LANATEST-SP01]`
...
**Depends on:**
- `_SPEC_SELFTEST.md [LANATEST-SP01]` for all FR/DD/IG/NFR requirements
...
- Zero changes to `src/lana/` (LANATEST-DD-01)
- Budget check BEFORE each live test (LANATEST-IG-01)
- Missing key = skip, not crash (LANATEST-IG-02)
...
**LANATEST-IP01-DC-01: Interpreter discovery (resolves LANATEST-PR-0004)**
...
**LANATEST-IP01-DC-02: Single-file script, importable module**
...
**LANATEST-IP01-DC-03: Category registry as data**
...
**LANATEST-IP01-DC-04: Unpriced models count $0 toward budget**
...
**LANATEST-IP01-DC-05: Workspace resolution**
...
- **LANATEST-IP01-EC-01**: No API key for any provider -> all live tests skip, offline categories still run, exit 0
- **LANATEST-IP01-EC-02**: Invalid category code argument -> print valid codes, exit 2
- **LANATEST-IP01-EC-03**: `import lana` fails -> print interpreter hint (LANA_PYTHON), exit 3
- **LANATEST-IP01-EC-04**: Model missing from pricing file -> cost null, warning, budget +$0, continue (DC-04)
- **LANATEST-IP01-EC-05**: Budget exhausted mid-run -> remaining live tests status `budget_exceeded`, results.json written, exit per pass/fail rule
- **LANATEST-IP01-EC-06**: KeyboardInterrupt -> finally-block writes results.json with completed tests, exit 130
- **LANATEST-IP01-EC-07**: Per-model timeout -> status fail with duration, continue next model
- **LANATEST-IP01-EC-08**: Available model matches no prefix entry -> status skip with warning (FR-06)
- **LANATEST-IP01-EC-09**: `.lana-data/` not writable -> print error, exit 3 (before any API call)
- **LANATEST-IP01-EC-10**: Provider endpoint unreachable in category 01 -> that check fails, run continues
- **LANATEST-IP01-EC-11**: Tool call category: model answers without tool_call -> status fail "no tool_call emitted"
- **LANATEST-IP01-EC-12**: `--model <id>` not in registry or not available -> print error, exit 2
- **LANATEST-IP01-EC-13**: Transient API error (429, 5xx) -> one retry after 3s (NFR-03), second failure = fail
- **LANATEST-IP01-EC-14**: Config files missing/malformed in category 02 -> checks fail with ConfigError text, run continues

## 4. Implementation Steps

### LANATEST-IP01-IS-01: Create workflow file
...
### LANATEST-IP01-IS-02: Create skill documentation
...
### LANATEST-IP01-IS-03: Script scaffold - categories, menu, arg parsing
...
### LANATEST-IP01-IS-04: Offline categories 01-03
...
### LANATEST-IP01-IS-05: Model discovery + shared turn runner
...
### LANATEST-IP01-IS-06: Category 04 Model Sweep
...
### LANATEST-IP01-IS-07: Category 05 Effort Matrix
...
### LANATEST-IP01-IS-08: Category 06 Tool Calls
...
### LANATEST-IP01-IS-09: Cost tracking, results, exit codes
...
### LANATEST-IP01-IS-10: Offline tests
...
- **LANATEST-IP01-TC-01**: `--menu` output contains all 6 codes with cost class -> ok
- **LANATEST-IP01-TC-02**: `select_categories(["01","04"])` -> exactly those two, ascending
- **LANATEST-IP01-TC-03**: `offline` -> 01-03; `live` -> 04-06; `all` -> 01-06
- **LANATEST-IP01-TC-04**: invalid code `99` -> exit 2, message lists valid codes

### Category 2: Discovery (3 tests)

- **LANATEST-IP01-TC-05**: registry fixture with disabled/unavailable models -> only enabled+available returned
- **LANATEST-IP01-TC-06**: model without prefix match -> excluded, warning recorded (EC-08)
- **LANATEST-IP01-TC-07**: provider without key -> models listed with skip status (EC-01)

### Category 3: Offline categories (3 tests)

- **LANATEST-IP01-TC-08**: category 02 on valid tmp workspace config -> all checks pass
- **LANATEST-IP01-TC-09**: category 02 with missing pricing file -> check fails, no exception (EC-14)
- **LANATEST-IP01-TC-10**: category 03 with skill folder missing SKILL.md -> check fails, named folder in message

### Category 4: Results and budget (3 tests)

- **LANATEST-IP01-TC-11**: results.json written with summary counts matching test list (IG-05)
- **LANATEST-IP01-TC-12**: budget precheck: remaining < $0.01 -> `budget_exceeded` status (EC-05)
- **LANATEST-IP01-TC-13**: exit code 1 when any fail, 0 when only pass/skip (IG-03)

## 7. Verification Checklist

### Prerequisites
- [ ] **LANATEST-IP01-VC-01**: SP01 read, all FR/IG cross-checked against steps
- [ ] **LANATEST-IP01-VC-02**: No `src/lana/` file modified (DD-01)

### Implementation
- [ ] **LANATEST-IP01-VC-03**: IS-01 workflow created
- [ ] **LANATEST-IP01-VC-04**: IS-02 SKILL.md created
- [ ] **LANATEST-IP01-VC-05**: IS-03..09 selftest.py complete
- [ ] **LANATEST-IP01-VC-06**: IS-10 offline tests created

### Validation
- [ ] **LANATEST-IP01-VC-07**: All offline test cases TC-01..13 pass
- [ ] **LANATEST-IP01-VC-08**: Manual: `/selftest` menu flow in Lana (scripted or live)
- [ ] **LANATEST-IP01-VC-09**: Manual: `selftest.py offline` green on dev checkout
- [ ] **LANATEST-IP01-VC-10**: Live run `selftest.py 04` within budget, results.json valid (user-approved spend)

## 8. Document History

**[2026-08-30 22:32]**
- Initial implementation plan created
- DC-01 resolves LANATEST-PR-0004 (interpreter discovery: LANA_PYTHON > .venv > PATH)
