...
def run_test(test_dir: Path, run_dir: Path, config: dict, args) -> dict:
  ...
  record_dir = run_dir / test_dir.name
  ...
  if 3 in metadata["tiers"] and rubric_path.exists() and not args.skip_judge:
    ...
    quality = evaluate_quality(workdir, manifest, rubric_path, record_dir / "judge", config)
...
def main() -> int:
  ...
  if not tests:
    ...
    return 2
  run_dir = RUNS_DIR / f"{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')}_{args.scope.replace('/', '-')}"
  suffix = 1
