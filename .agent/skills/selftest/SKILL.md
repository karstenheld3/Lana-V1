...
- **01 Environment** (offline, free): Python >= 3.12, Lana version, `.agent-data/` writable, provider endpoints reachable (TLS connect, no auth)
- **02 Configuration** (offline, free): `agent-config.json` valid, roles resolve, registry/mapping/pricing parse, API keys present, pricing coverage
- **03 Prompt System** (offline, free): `.agent/` folders, workflow frontmatter, `SKILL.md` per skill
...
- Runs from the workspace root (uses cwd; honors `AGENT_CONFIG` env var like Lana itself)
...
- Results: stdout progress + `.agent-data/selftest/<YYYY-MM-DD_HH-MM-SS>/results.json` (written even on interrupt)
