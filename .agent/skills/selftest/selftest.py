...
Runs from the workspace root. Results: stdout + .agent-data/selftest/<timestamp>/results.json (written even on interrupt, IG-04).
...
def environment_checks(ctx: Context) -> list:  # injectable for tests (TP01-TC-17)
  ...
  def data_dir():
    target = ctx.workspace / ".agent-data"
...
def run_configuration(ctx: Context) -> None:
  ...
  try:  # EC-14: any config problem fails the check, never crashes the run
    app = load_lana_config(ctx.workspace, ctx.config_dir / "agent-config.json", require_keys=False)
    record(ctx, "02", "config_and_roles", "pass", time.monotonic() - started)
    print(f"  agent-config.json: valid...OK | roles: {len(app.roles)} resolved...OK")
  except Exception as error:
...
def run_prompt_system(ctx: Context) -> None:
  agent = ctx.workspace / ".agent"
  for sub in ("rules", "workflows", "skills"):
    exists = (agent / sub).is_dir()
    record(ctx, "03", f"folder_{sub}", "pass" if exists else "fail", 0.0, None if exists else f".agent/{sub}/ missing")
    print(f"  .agent/{sub}/: {'OK' if exists else 'FAIL - missing'}")
...
def tool_call_models(ctx: Context, models: list[dict]) -> list[dict]:  # DD-06: configured generator per provider, fallback cheapest
  generator_id = None
  try:
    generator_id = read_json(ctx.config_dir / "agent-config.json").get("roles", {}).get("generator", {}).get("model_id")
  except Exception:
    pass
  selected = []
...
def main(argv: Optional[list] = None) -> int:
  ...
  try:
    ...
    return 2
  workspace = Path.cwd()
  config_override = os.environ.get("AGENT_CONFIG")
  config_dir = Path(config_override).parent if config_override else workspace / ".agent-data" / "config"
  ...
  run_dir = workspace / ".agent-data" / "selftest" / datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
