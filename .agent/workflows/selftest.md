...
- `/selftest` (no arguments) -> run `<python> .agent/skills/selftest/selftest.py --menu`, show the menu to the user, ask which categories to run. Wait for the answer.
...
```
<python> .agent/skills/selftest/selftest.py <codes> [--provider P] [--model M] [--budget N] [--timeout S]
```

Run blocking from the workspace root. The script prints progress per category and writes `results.json` to `.agent-data/selftest/<timestamp>/`.
