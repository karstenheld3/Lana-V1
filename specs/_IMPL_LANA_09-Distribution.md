...
**Depends on:**
- `_SPEC_LANA_09-Distribution.md` [LANADIST-SP01] for pipeline requirements, guarantees, logging contract
...
# $env:PYAPP_PASS_LOCATION='1'                    # PYAPP env var = outer exe path at runtime (DD-11)
...
**Note**: `PYAPP_EXEC_MODULE='lana'` instead of SPEC's earlier `PYAPP_EXEC_SPEC='lana.cli:main'`: the module path runs `__main__.py` which wraps `main()` in `sys.exit()` - exit codes propagate to the ACP client. `EXEC_SPEC` would discard the return value. SPEC updated (see LANADIST-SP01 history 2026-08-30 16:25). `PYAPP_PASS_LOCATION='1'` is a compile-time option that makes PyApp inject the outer exe's absolute path as the `PYAPP` env var at runtime (default is just `"1"`); required for `resolve_app_dir()` portable-app auto-detection (DD-11).
...
- Added: boot path debug log (`dlog "app" "paths"`) in `build_runtime()` for diagnosing data_dir resolution
- Changed: `--install-root` renamed to `--app-dir` throughout (`cli.py`, `config.py`, `acp/server.py`)
- Synced from code: `resolve_app_dir()` reads `PYAPP` env var for auto-detection
