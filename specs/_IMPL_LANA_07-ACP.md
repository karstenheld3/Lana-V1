...
**Depends on:**
- `_SPEC_LANA_07-ACP.md [LANAACPB-SP01]` for all wire shapes, FRs, DDs, IGs
- `_SPEC_LANA_02-AgentCore.md [LANACORE-SP01]` for Agent core contracts (FR-04 loop, FR-08 full recall, FR-12 approvals)
...
### Hardening (absorbed from `_IMPL_LANA_HARDENING.md [LANAAGNT-IP02]` Phase 5)

Phase 5 of `_IMPL_LANA_HARDENING.md [LANAAGNT-IP02]` implements ACP-specific hardening:

- **LANAAGNT-IP02-IS-13**: `acp/jsonrpc.py` - `StdoutWriter` thread with bounded queue (default 1000); overflow drops with stderr log; `close()` drains on shutdown; injected `write_line` unaffected
- **LANAAGNT-IP02-IS-14**: `acp/server.py` - `build_session_runtime` via `run_in_executor` (stdout redirect inside the callable); cancel paths and EOF shutdown call `terminate_tool_processes` per session

Edge cases EC-09..11 and test cases TC-13..16 in `LANAAGNT-IP02` cover writer queue overflow, cancel with live processes, exit with background processes, and ACP regression verification.
