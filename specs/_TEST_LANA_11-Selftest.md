...
**Doc ID**: LANASTST-TP01
**Goal**: Verify the selftest framework meets all LANASTST-SP01 requirements -- offline via pytest, live via controlled selftest runs
...
**Depends on:**
- `_SPEC_LANA_11-Selftest.md [LANASTST-SP01]` for requirements
- `_IMPL_LANA_11-Selftest.md [LANASTST-IP01]` for implementation structure and edge cases
...
Offline cases TC-01..13 correspond 1:1 to `LANASTST-IP01-TC-01..13`; TC-17..19 supplement coverage gaps found during /verify. All offline cases implemented in `tests/test_selftest_script.py`:

### Category 1: Selection and menu (4 tests)

- **LANASTST-TP01-TC-01**: `--menu` lists codes 01-06 with cost class and estimates -> ok=true (FR-01)
- **LANASTST-TP01-TC-02**: codes `["04","01"]` -> selected `[01, 04]` ascending -> ok=true (FR-02)
- **LANASTST-TP01-TC-03**: `offline` -> 01-03, `live` -> 04-06, `all` -> 01-06 -> ok=true (FR-02)
- **LANASTST-TP01-TC-04**: code `99` -> ok=false, exit 2, message lists valid codes (EC-02)

### Category 2: Model discovery (3 tests)

- **LANASTST-TP01-TC-05**: fixture registry with disabled + untested models -> only enabled+available returned (FR-06)
- **LANASTST-TP01-TC-06**: model without prefix entry -> excluded with warning (EC-08)
- **LANASTST-TP01-TC-07**: no ANTHROPIC_API_KEY -> anthropic models status skip, openai unaffected (EC-01, IG-02)

### Category 3: Offline categories (3 tests)

- **LANASTST-TP01-TC-08**: valid tmp workspace -> category 02 all checks pass (FR-04)
- **LANASTST-TP01-TC-09**: pricing file deleted -> category 02 check fails, no exception, run continues (EC-14)
- **LANASTST-TP01-TC-10**: skill folder without SKILL.md -> category 03 fails naming the folder (FR-05)

### Category 4: Results, budget, exit codes (3 tests)

- **LANASTST-TP01-TC-11**: full offline run -> results.json valid JSON, summary counts match tests array (IG-04, IG-05)
- **LANASTST-TP01-TC-12**: budget precheck with remaining < $0.01 -> `budget_exceeded`, no API attempt (FR-10, IG-01)
- **LANASTST-TP01-TC-13**: one fail -> exit 1; only pass+skip -> exit 0 (IG-03)

### Category 5: Live end-to-end (3 manual cases, user-approved spend)

- **LANASTST-TP01-TC-14**: `selftest.py 04 --model gpt-5-nano --budget 0.10` -> pass, cost < $0.01, results.json entry complete (FR-07)
- **LANASTST-TP01-TC-15**: `selftest.py live --budget 1.00` -> categories 04-06 complete, all 20 models attempted, effort matrix covers 5 methods, tool calls pass (FR-07..09; validates NFR-02 estimates and closes LANASTST-PR-0001/PR-0002)
- **LANASTST-TP01-TC-16**: `/selftest` in Lana (menu flow) -> menu presented, selection executed, summary reported (FR-14)

### Category 6: Supplementary unit tests (3 tests)

- **LANASTST-TP01-TC-17**: category 01 offline checks (python version, lana version, data dir writable) pass in tmp workspace; endpoint checks excluded via injectable check list (FR-03)
- **LANASTST-TP01-TC-18**: `run_model_turn()` with stub adapter that stalls -> status fail with timeout message after configured 1s, next test proceeds (FR-13, EC-07)
- **LANASTST-TP01-TC-19**: effort matrix selection on fixture registry + pricing -> cheapest model per method chosen, effort levels derived from prefix array or low/medium/high default (FR-08)
...
### Offline (pytest)
- [ ] **LANASTST-TP01-VC-01**: TC-01..13 and TC-17..19 implemented in `tests/test_selftest_script.py`
- [ ] **LANASTST-TP01-VC-02**: `pytest tests/test_selftest_script.py -q` green without API keys in env
- [ ] **LANASTST-TP01-VC-03**: Full offline suite `pytest -m "not live"` -- no new failures beyond deferred PR-0005

### Live (user-approved)
- [ ] **LANASTST-TP01-VC-04**: TC-14 live smoke green, cost reported
- [ ] **LANASTST-TP01-VC-05**: TC-15 full live run -- results reviewed, failing models recorded in PROBLEMS.md
- [ ] **LANASTST-TP01-VC-06**: TC-16 `/selftest` menu flow works in Lana
- [ ] **LANASTST-TP01-VC-07**: NFR-02 cost estimates compared against actuals, spec updated if off

### Coverage cross-check
- [ ] **LANASTST-TP01-VC-08**: Every SP01 FR has at least one TC (FR-01..14 -> TC map in sections 6)
- [ ] **LANASTST-TP01-VC-09**: Session goal met -- all available models tested, effort variants covered (PR-0001, PR-0002 closable)
