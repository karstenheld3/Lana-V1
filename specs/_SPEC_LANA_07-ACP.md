...
**Depends on:**
- `_SPEC_LANA_02-AgentCore.md [LANACORE-SP01]` for AgentEvent stream (DD-06), full-recall sessions (FR-08, DD-22), slash expansion (FR-05, DD-13), ExecutionPolicy (FR-12, DD-15)
- `_SPEC_LANA_06-CLI.md [LANACLI-SP01]` for cost tracking (FR-09), headless/test interfaces (FR-14, DD-20)
...
- `prompt_step` → no ACP mapping (headless-only event, moved to `_SPEC_LANA_06-CLI.md [LANACLI-SP01]`; never occurs in ACP mode)
...
- Session files remain in `<app_dir>/.lana-data/sessions/` (DD-25); the `cwd` from `session/new` is the workspace for tool context and git-root detection
- `available_commands_update` sources the loaded PromptSystem; built-ins (`/help`, `/cost`, `/exit`) are CLI-only and not advertised

## 13. Document History

**[2026-09-01 21:50]**
- Renamed from `_SPEC_LANA_MVP-2_ACP.md` to `_SPEC_LANA_07-ACP.md` (spec restructure step 8)
- Removed: LANAACPB-FR-12 (Prompt Queue), PromptQueueFile domain object, LANAACPB-DD-10, prompt_step/PromptQueueFile data structures - moved to `_SPEC_LANA_06-CLI.md [LANACLI-SP01]`
- Changed: Dependencies updated to reference new spec filenames (02-AgentCore, 06-CLI)
- Changed: Goal shortened (prompt queue clause removed)
